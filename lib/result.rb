# класс хранящий и отдающий результаты теста
class Result
  ...
  def print
    puts "Вы набрали #{@score} баллов"

    if (30..32).cover?(@score)
      puts @results[0]
    elsif (25..29).cover?(@score)
      puts @results[1]
    elsif (19..24).cover?(@score)
      puts @results[2]
    elsif (14..18).cover?(@score)
      puts @results[3]
    elsif (9..13).cover?(@score)
      puts @results[4]
    elsif (4..8).cover?(@score)
      puts @results[5]
    elsif @score <= 3
      puts @results[6]
    else
      puts 'Вы набрали странное кол-во очков, советую вам разобраться в себе или в программе'
    end
  end
end
